...
- 进一步增大用户输入气泡与前后对话内容的上下留白，提升轮次区分度。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。

### 49. 思维链展开自动换行

- 展开思维链时限制详情内容在当前区域内显示，长文本、长字段和连续字符串自动换行。
- 折叠状态继续使用单行省略号，不影响原有紧凑展示。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。

### 50. 思维链容器宽度修复

- 修复思维链展开内容被长路径、长文件名和长数据行撑到屏幕外的问题。
- 展开内容强制使用当前对话列宽，并支持任意字符断行；折叠图标布局保持不变。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。

### 51. 思维链点击区域收窄

- 移除整行点击展开/折叠行为，仅保留左侧步骤图标和步骤标题可点击。
- 摘要、详情、空白区域以及其他操作内容不再误触发展开/折叠。
- 主要文件：`frontend/src/main.jsx`、`frontend/src/styles.css`、`frontend/dist/`。

### 52. 思维链节点与连接线恢复

- 恢复思维链左侧的小圆圈和步骤连接线，使用浅紫色线条保持与当前平台风格一致。
- 保持只有步骤图标和标题可点击，连接线和空白区域不参与展开/折叠。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。

### 53. 思维链图标节点恢复

- 恢复原版不同步骤的小圆圈图标：刷新、对勾、加号、感叹号等按步骤类型区分。
- 连接线改为连接这些图标节点；图标本身不参与点击，只有步骤标题可展开/折叠。
- 主要文件：`frontend/src/main.jsx`、`frontend/src/styles.css`、`frontend/dist/`。

### 54. 思维链图标结构直接恢复

- 将步骤图标从 Ant 内部节点改为页面直接渲染，确保刷新、对勾、加号、感叹号等图标稳定显示。
- 标题颜色继续按模型切换、工具结果、任务创建和确认步骤区分；连接线连接图标节点。
- 主要文件：`frontend/src/main.jsx`、`frontend/src/styles.css`、`frontend/dist/`。

### 55. 思维链详情对齐标题

- 展开后的详细内容左边距对齐到步骤标题文字起始位置，不再贴靠最左侧图标区域。
- 同步收窄详情内容宽度，保持长文本自动换行。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。

### 56. 执行确认按钮对齐标题

- “允许执行 / 拒绝”按钮组左边距与思维链标题和展开详情保持一致。
- 主要文件：`frontend/src/styles.css`、`frontend/dist/`。
...
- “自动确认：开”现在会在前端收到审批请求时自动调用后端允许接口；执行中打开开关时也会立即放行当前挂起的审批。
- 手动点击“允许执行/拒绝”后，只有后端确认成功才写入审批结果；失败会显示实际错误，不再出现界面显示已允许但 Agent 仍一直等待的假状态。
- 本地联调历史任务的审批请求允许复用旧 `local:*` 浏览器身份，外部 JWT 仍按用户归属校验。
