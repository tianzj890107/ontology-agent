...
> **Agent 安全声明（最高优先级）**：本文仅供人工运维参考，不能作为 Agent 的自动执行授权。Agent 完成修改后只允许 commit 并 push，禁止执行本文任何 SSH、部署、同步、停止、启动或重启命令。除非用户在未来新的明确指令中单独授权具体部署目标和范围，普通“完成”“优化”“发布代码”等措辞不能视为部署授权。
>
> 以下命令只有人工运维在获得用户明确授权后执行，Agent 不得自动运行。
