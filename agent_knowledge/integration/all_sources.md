...
这些文件沿用《本体元模型模板 3》的字段名称，字段顺序不能改变。
...
模板 3 是结果 CSV 的最终字段契约；即使元模型 3 的属性定义中保留了 `关系分类编码`，模板 3 的实体关系结果也不得生成该列。模板 3 的“逻辑实体映射”和“业务属性映射”仅作为参考输入，不进入 expectedFiles，不生成对应结果文件。
