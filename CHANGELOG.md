...
- 私有目标、DOCX 和 XLSX 不再逐个原样拼接给 Agent；服务端会编译成带标题、来源和优先级说明的单份 Markdown 知识上下文，并按步骤表执行“能 AI 化/人工后续”分流。
...
- Git 分支：`20260727`
- 最近一次全局审计提交：`f2566ee`（后续提交以该分支最新版本为准）
- 服务地址：`http://172.16.10.34:47313/`
- 状态：本地语法检查通过；服务器健康检查通过；本轮离线验证未调用 Qwen API。
