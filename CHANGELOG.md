...
### 21. Qwen 配额不足自动切换同类模型

- Qwen 返回配额不足、限流、余额不足或 HTTP 429 时，自动从当前模型所属类别切换到其他已配置模型。
- 视觉模型只在 `QWEN_VISION_MODELS` 中切换，文本模型只在 `QWEN_TEXT_MODELS` 中切换，避免能力类型错配。
- 最多按配置顺序尝试 8 个同类模型；全部失败后才向用户显示原始错误。
- 页面会显示“当前模型额度不足，已自动切换到……”并同步当前模型名称。
- 主要文件：`open-claude/open_claude/openai_compat.py`、`open-claude/oc_codex_server.py`、`open-claude/codex_web.html`。
- 类型：Qwen 模型调用逻辑与前端提示变更，需要重启后端服务。

### 20. 主链稳定性审计与部署地址修正

- 任务上下文相同的后续对话不再重复下载对象存储文件，避免每轮调用都产生重复网络请求和重复文件。
- 远程输入文件用 `objectKey` 摘要区分同名来源，重启后仍落到同一路径；修复文件名可能配错前一个对象 Key 的问题。
- 任务进入“执行中”及写入首条用户消息后立即保存状态；状态文件写入增加单独锁，避免多任务并发覆盖。
- 前端补齐接口非 200 和非 JSON 响应的错误处理，避免请求失败后没有明确提示。
- 修复重启恢复时先创建 Agent、后设置沙箱边界的顺序问题。
- 智能建模主页面的小机器人地址改为使用当前页面的服务器主机名；服务器部署时不再错误连接访问者本机的 `127.0.0.1`。
...
- 保留自动确认开启时的绿色按钮样式。
- 鼠标悬停在已开启按钮上时不再切换为橙色，继续保持绿色。
- 涉及页面：`open-claude/codex_web.html`。
