...
class StaticKnowledgeContractTests(unittest.TestCase):
    ...
    def test_runtime_reads_markdown_only(self):
        ...
        self.assertIn("自底向上业务对象识别规范_v3.md", modeling)
        self.assertIn("ER 连通岛", modeling)
        self.assertIn("最高优先级建模规范", modeling)
        ...
        v3_rules = (ROOT / "agent_knowledge" / "modeling" / "自底向上业务对象识别规范_v3.md").read_text(encoding="utf-8")
        self.assertIn("自底向上业务对象识别与逻辑实体聚合规范 V3", v3_rules)
        self.assertIn("业务对象成岛规则", v3_rules)
