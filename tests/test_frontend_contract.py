...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_mission_file_panel_draws_available_ontology_layers(self):
        ...
        self.assertIn('id: "ontology:unassigned-entities"', source)
        self.assertIn('type: "graph"', source)
        self.assertIn('layout: "none"', source)
        self.assertIn('const attributeColumns = 4;', source)
        self.assertIn('[Math.round(node.symbolSize[0] * 1.3), 50]', source)
        self.assertIn('objectX = Math.max(padding + objectWidth / 2, width * 0.25);', source)
        self.assertIn('entityX = Math.min(width - padding - entityWidth / 2, width * 0.75);', source)
        self.assertIn('const gap = (width - padding * 2 - columnWidths.reduce', source)
        self.assertIn('data: [...graphNodes, ...layoutAnchors]', source)
        self.assertIn('links: graphLinks', source)
