...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_uses_thought_chain_and_existing_api_routes(self):
        ...
        self.assertIn('import { DownOutlined, SettingOutlined } from "@ant-design/icons";', source)
        self.assertIn('<SettingOutlined /> 大语言模型设置', source)
        self.assertIn('<DownOutlined className="model-arrow" />', source)
        ...
        self.assertIn('.thought-icon-read-file svg,.thought-icon-write-file svg,.thought-icon-edit-file svg{width:12px;height:12px}', styles)
