...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_mission_file_panel_draws_available_ontology_layers(self):
        ...
        # 径向分层多轨道布局：使用 ECharts Graph + layout none，不使用 Tree
        self.assertIn('type: "graph"', source)
        self.assertIn('layout: "none"', source)
        self.assertNotIn('type: "tree"', source)
        # 隐藏技术根已移除：未归属实体作为普通 entity 根节点，不创建虚拟组
        self.assertNotIn('id: "ontology:unassigned-entities"', source)
        self.assertNotIn('virtualGroup', source)
        # 布局纯函数模块接入
        self.assertIn('import { layoutOntologyRadial } from "./ontologyRadialLayout.js";', source)
        layout_module = (ROOT / "frontend" / "src" / "ontologyRadialLayout.js").read_text(encoding="utf-8")
        self.assertIn('export function computeSectorWeights(', layout_module)
        ...
        self.assertIn('export function computeNodeAngle(', layout_module)
        self.assertIn('export function polarToCartesian(', layout_module)
        self.assertIn('export function layoutOntologyRadial(', layout_module)
        # 扇区权重与后代节点数量成正比、业务属性层支持多轨道、节点安全间距
        self.assertIn('sum + 1 + (attributesByEntity.get(entity.id) || []).length', layout_module)
        self.assertIn('computeTrackCount(', layout_module)
        self.assertIn('const slot = maxNodeWidth + minGap;', layout_module)
        # 没有业务对象时逻辑实体作为最内层
        self.assertIn('computeRingRadius(0, 0, maxEntityWidth, minGap, 0)', layout_module)
        # 只生成真实连线：业务对象->逻辑实体、逻辑实体->业务属性
        self.assertIn('links.push({ source: group.object.id, target: item.id })', layout_module)
        self.assertIn('links.push({ source: entity.id, target: attribute.id })', layout_module)
        # 首尾节点不被画布边界裁切：外圈预留节点半径与悬浮放大空间
        self.assertIn('requiredRadius = outerRadius + maxOuterHalfWidth * hoverScale + padding', layout_module)
        self.assertIn('Math.max(width, requiredRadius * 2)', layout_module)
        self.assertIn('Math.max(height, requiredRadius * 2)', layout_module)
        # 业务属性默认隐藏，全局展开/隐藏按钮
        self.assertIn('showAttributes: showAttributes && hasAttributes', source)
        ...
        # 交互：触控板双指平移、捏合缩放（roam + ctrlKey 放行）、拖拽平移、ResizeObserver 重算
