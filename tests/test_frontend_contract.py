...
class FrontendContractTests(unittest.TestCase):
    ...
    def test_workbench_uses_thought_chain_and_existing_api_routes(self):
        ...
        self.assertIn('const cachedSession = standaloneSessionCacheRef.current.get(runId);', source)
        self.assertIn('events: cachedEvents,', source)
        self.assertIn('files: cachedSession?.files?.length ? cachedSession.files : (cached?.files || []),', source)
        ...
        self.assertIn('const displayEvents = normalizeEvents({ events });', source)
        self.assertIn('const approvalResults = displayEvents.reduce((result, event) =>', source)
        ...
        self.assertIn('const executionFinished = event.type === "approval_request" && displayEvents.slice(index + 1).some((candidate) => candidate.type === "done");', source)
        ...
        self.assertIn('durationMs={eventDuration(displayEvents, index)}', source)
    ...
    def test_mission_file_panel_draws_available_ontology_layers(self):
        ...
        source = (ROOT / "frontend" / "src" / "main.jsx").read_text(encoding="utf-8")
        styles = (ROOT / "frontend" / "src" / "styles.css").read_text(encoding="utf-8")
        graph_model = (ROOT / "frontend" / "src" / "ontologyGraphModel.js").read_text(encoding="utf-8")
        force_layout = (ROOT / "frontend" / "src" / "ontologyForceLayout.js").read_text(encoding="utf-8")
        ...
        self.assertIn("sigma", package["dependencies"])
        self.assertIn("graphology", package["dependencies"])
        self.assertIn("graphology-layout-forceatlas2", package["dependencies"])
        self.assertIn('import { buildOntologyGraph } from "./ontologyGraphModel.js";', source)
        self.assertIn('export function buildOntologyGraph(records)', graph_model)
        ...
        self.assertIn('entityRelation: ["entity_relations.csv", "entity_relationships.csv"]', source)
        ...
        self.assertIn('action: ["actions.csv"]', source)
        self.assertIn('return selected.has("logicalEntity") ? selected : null;', source)
        precompute_module = (ROOT / "frontend" / "src" / "ontologyRadialPrecompute.js").read_text(encoding="utf-8")
        self.assertIn('type: "graph"', precompute_module)
        self.assertIn('layout: "none"', precompute_module)
        self.assertNotIn('type: "tree"', source + precompute_module)
        layout_module = (ROOT / "frontend" / "src" / "ontologyRadialLayout.js").read_text(encoding="utf-8")
        self.assertIn('export const ONTOLOGY_LAYER_DEFINITIONS = [', graph_model)
        self.assertIn('export { ONTOLOGY_LAYER_DEFINITIONS } from "./ontologyGraphModel.js";', layout_module)
        ...
        self.assertIn('export function packSemanticLayer(', layout_module)
        self.assertIn('export function compactTrackRadii(', layout_module)
        self.assertIn('export function calculateNaturalBounds(', layout_module)
        self.assertIn('export function layoutQualityMetrics(', layout_module)
        ...
        self.assertIn('handleApplyLayers(ONTOLOGY_LAYER_DEFINITIONS.map', source)
        self.assertIn('>确认</Button>', source)
        self.assertIn('disabled={!availability[layer.key]}', source)
        self.assertIn('{ key: "entityRelation", label: "实体关系"', graph_model)
        self.assertIn('{ key: "metric", label: "指标"', graph_model)
        self.assertIn('{ key: "action", label: "动作"', graph_model)
        self.assertIn('.ontology-sigma-shell{border:0;border-radius:0}', styles)
        self.assertIn('className="ontology-layer-filter-button"', source)
        self.assertIn('fillRule="evenodd"', source)
        self.assertIn('nodeScaleRatio: 1', precompute_module)
        self.assertIn('const displayScale = layoutPrepared.fitScale;', source)
        self.assertIn('viewportWidth: width', precompute_module)
        self.assertIn('viewportHeight: height', precompute_module)
        self.assertIn('13 * displayScale * Math.min(Math.max(0.1, roamZoom), 1.8)', source)
        ...
        self.assertIn('roam: true', precompute_module)
        self.assertIn('observer = new ResizeObserver(() => {', source)
        self.assertIn('emphasis: { focus: "none", scale: 1.12 }', precompute_module)
        self.assertIn('selectedMode: false', precompute_module)
        ...
        # 语义环形布局（ECharts）在后台完成布局数据计算并缓存，切换时直接复用。
        ...
        self.assertIn('<OntologyEChartsPreview key={`radial:${layerKey}`} data={data} appliedLayers={appliedLayers} prepared={radialPrepared} onError={handleLayoutError} onRendered={handleLayoutRendered} onViewportChange={handleRadialViewportChange} />', source)
        self.assertIn('<OntologySigmaPreview data={data} appliedLayers={appliedLayers} onError={handleLayoutError} onRendered={handleLayoutRendered} />', source)
        # Sigma 异步初始化错误通过回调进入统一失败处理，成功渲染后才更新 lastGood。
        self.assertIn('onError?.(error);', sigma_preview)
        self.assertIn('onRendered?.("network");', sigma_preview)
        self.assertIn('handleLayoutRendered', source)
    ...
    def test_ontology_preview_unified_layout_selector(self):
        ...
        # 语义环形布局真实后台预计算：空闲时准备布局数据并缓存，切换时复用缓存。
        self.assertIn('createRadialLayoutCache', source)
        self.assertIn('prepareRadialLayout(data, appliedLayers, viewport)', source)
        self.assertIn('radialCacheKey', source)
        self.assertIn('prepared={radialPrepared}', source)
        self.assertIn('layoutIsForViewport', source)
        self.assertIn('onRendered?.("radial")', source)
        # 成功渲染后才把当前布局标记为 lastGood；切换本身不提前标记。
        self.assertNotIn('lastGoodLayoutRef.current = layoutMode;', source)
        self.assertIn('handleLayoutRendered', source)
    ...
    def test_task_events_endpoint_and_active_run_exists_contract(self):
        ...
        self.assertIn('response["model"] = task.effective_model()', server)
        self.assertIn('let liveModel = result.model;', source)
        self.assertIn('if (!liveModel) {', source)
        self.assertIn('model: liveModel,', source)
