...
- 数据与部署：部署前将 `.web_tasks.json` 和完整 `.task_history` 备份到服务器 `open-claude/sandbox/session-recovery-backup-20260825-1800`（约 62MB、35 个 journal），未改写原始历史；修复提交 `b049e13` 已部署到 47313（pid `2983038`）。全量比较 43 个任务的 API 与磁盘有效事件数：35 个有历史会话全部一致、8 个无 journal 且历史快照也无事件的空任务保持为空、0 个失败；最大会话 433,942 条，尾页正常返回 80 条，`/health` 为 ready。
...
- 100px 左移提交 `b1d0540` 已推送并部署：47313 pid `2836156`、47314 pid `2837201`；发布门禁 20 个测试通过，前端生产构建通过（仅保留既有大包体积提示），线上主 bundle 更新为 `index-wuFNXe6_.js`。
- 本体预览由带隐藏技术根的 Tree 改为固定坐标 Graph，只绘制真实层级关系，消除展开属性后业务对象/逻辑实体区域的幽灵连线；业务对象和逻辑实体始终展示且节点点击不再折叠，悬浮仅保留当前节点放大、不再淡化其他节点。新增统一的“隐藏业务属性 / 展开业务属性”按钮，属性随预览宽度自动按 2～4 列排布并动态计算分组高度。
- 静态本体图提交 `f2ee2b6` 已推送并部署：47313 pid `2875504`、47314 pid `2876601`；发布门禁 20 个测试通过，前端生产构建通过（仅保留既有大包体积提示），线上主 bundle/CSS 更新为 `index-DQg0GIPc.js` / `index-Dhb9cv_q.css`。
- 调整静态图密度：隐藏业务属性时将保留的业务对象/逻辑实体椭圆宽度放大 30%、高度增至 50px、字号增至 15px；展开业务属性时固定采用四列属性布局，预览宽度不足时通过横向滚动完整展示。
- 密度调整提交 `83732f0` 已推送并部署：47313 pid `2898921`、47314 pid `2900089`；发布门禁 20 个测试通过，前端生产构建通过（仅保留既有大包体积提示），线上主 bundle 更新为 `index-B3YAIGJV.js`。
- 本体图改为展开/隐藏两套独立响应式横向布局：隐藏属性时按画布实时宽度将业务对象、逻辑实体定位在约 1/4 与 3/4；展开时统计每列最大椭圆宽度，在不重叠前提下动态压缩业务对象、逻辑实体和四列属性的间距，空间不足才扩宽画布并滚动。新增不可见边界锚点仅用于保留左右气泡完整尺寸，避免首列和末列被 ECharts 绘图区裁切。
- 响应式布局提交 `f7d57bc` 已推送并部署：47313 pid `2942831`、47314 pid `2944091`；发布门禁 20 个测试通过，前端生产构建通过（仅保留既有大包体积提示），线上主 bundle/CSS 更新为 `index-BWH7cqPQ.js` / `index-I8FPupji.css`。
- 优化本体图触控板漫游：普通双指横向/纵向滑动转换为画布平移，阻止 ECharts 将滚动误判为缩放；触控板捏合/张开手势继续交由 ECharts 缩放，鼠标或触控板按住拖动仍可平移。
- 触控板漫游提交 `c1d083a` 已推送并部署：47313 pid `3025340`、47314 pid `3026339`；发布门禁 20 个测试通过，前端生产构建通过（仅保留既有大包体积提示），线上主 bundle 更新为 `index-PFNI1xBX.js`。
