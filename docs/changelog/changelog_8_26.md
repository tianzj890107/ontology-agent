...
- 部署：网络图现场修正提交 `d47e65b` 已推送并发布；部署前两套服务均无活动或排队任务，服务器门禁 20 项通过。47313 pid `4125884`、47314 pid `4127565`，两服务 `/`、`/health` 均为 200，线上主 JS/CSS 为 `index-tluNSlXh.js` / `index-Dkfcaex9.css`，Sigma chunk 为 `OntologySigmaPreview-eGLRKC69.js`；默认网络图、稀疏结果宽高比展开、筛选重建、后台预载环形资源及 Beta 文案移除均已上线，服务器既有运行数据文件保持原样。
