...
所有本体建模任务会自动在 `input/` 放入三份固定参考文件：`本体元模型v0.0.1.xlsx`、`本体元模型模板v.0.0.1.xlsx` 和 `本体元模型模板v0.0.1（含样例数据）.xlsx`。独立的 `Ontology平台模型编码规范` 不再作为任务输入；运行时编码契约规定业务对象编码为 `BO` + 4 位流水码，例如 `BO0001`。未带 `v0.0.1` 的历史文件不再作为新任务固定输入。`business_attributes.csv` 使用十六列表头，新增 `数据长度`、`数据精度`；无法取得或不适用时留空，不得猜测。`logical_entities.csv` 的 `是否主逻辑实体` 和业务属性布尔字段统一使用 `Y/N`；每个业务对象必须且只能有一个 `是否主逻辑实体=Y`。`是否唯一`表示业务上的唯一标识；复合业务唯一标识不得拆成多个单字段唯一。当前未实现维度输出时 `是否层级编码` 和 `是否层级名称` 全部填 `N`。同一逻辑实体同时存在 `XXX编码`（逻辑主键）和 `XXX名称` 时，`XXX名称` 的 `是否页面显示` 填 `Y`，其他业务属性填 `N`。模板的“逻辑实体映射”和“业务属性映射”仅作为参考输入，不进入结果文件清单。业务规则正式表头统一使用 `规则编码,规则名称,规则描述,触发条件,判断或结果,处置动作`；规则编码使用 `R` + 7 位流水码，例如 `R0000001`。状态和事件没有定义固定新编码前缀：已有稳定编码时沿用，无来源时标记待确认，不得自定格式。
